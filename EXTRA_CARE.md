...
### Подозрение: медленный фолбэк QUIC→TCP на отдельных клиентах (2026-05-12)

**Симптом**: эпизодически на одном из клиентов конкретный сайт долго грузится или «не открывается». Воспроизведено на `https://www.alfaromeosrbija.rs/modeli/giulia` (Akamai CDN, IP 2.16.6.x). С роутера через `proxy http://127.0.0.1:8118` и `direct` обе версии отдают HTTP/2 200 моментально, значит маршрут до сайта рабочий.

**Гипотеза**: правило `udp:443 → block` (QUIC-block для Windows-телеметрии) кладёт HTTP/3 handshake. Большинство браузеров мгновенно фолбэчат в HTTP/2 поверх TCP, но некоторые залипают на ретраях QUIC из-за HSTS-pinning, кэша или особенностей Akamai (длинный alt-svc заголовок с h3).

**Что делать если повторится**:
1. Проверить с роутера через `curl -skI --proxy http://127.0.0.1:8118 https://URL` — если 200, сайт жив, проблема на клиенте.
2. Открыть в инкогнито / другом браузере — отметает HSTS/кэш.
3. Если воспроизводится регулярно на конкретном домене: добавить exception в QUIC-block (`udp:443 to <IP> accept`) или в роутинг (`outboundTag: "direct"` для домена), не отключая block целиком.
4. Long-term: рассмотреть отказ от глобального `udp:443 block` в пользу более точечного блока win-spy QUIC по доменам.

Пока зафиксировано как подозрение, без правок.

### Попытка добавить XHTTP outbound в balancer — откатили (2026-05-12)

**Что делали**: на сервере подняли VLESS+XHTTP+TLS inbound (3x-ui id=6, порт 2087, ALPN h2, path `/xhttp`, mode auto, тот же UUID). На роутере добавили `proxy-xhttp` outbound — balancer (`selector: ["proxy-"]`) автоматически взял бы его в пул, observatory начал бы пинговать.

**Что пошло не так**: после `/etc/init.d/xray restart` интернет у клиентов пропал **на ~минуту** (не на секунды). Скорее всего клиентский `proxy-xhttp` outbound был сконфигурён неправильно, и **observatory завис на его пробе**: с `enableConcurrency: false` пробы идут по очереди, дохлый proxy-xhttp висел до таймаута → reality/grpc не успевали получить статус alive → balancer (`strategy: random`) не имел ни одного живого outbound'а → весь трафик дропался пока проба не отвалилась. Сид не дождался, ребутнул роутер. После ребута всё встало нормально (с XHTTP в конфиге — но это уже неважно). `proxy-xhttp` outbound удалён с роутера, конфиг откатан на pre-xhttp. Серверный inbound id=6 на 2087 оставлен спать (ничего не ест, удаляется через панель).

**Уроки**:
1. **Новый outbound НЕЛЬЗЯ добавлять в balancer, который гейтит весь трафик, не протестировав его в изоляции.** Сначала — отдельное routing-правило на один тестовый домен через новый outbound (мимо balancer'а), проверить что ходит. Только потом класть в `selector`. Иначе кривой outbound отравит observatory и положит весь VPN на время таймаута пробы.
2. **Rollback-скрипты в `/tmp` не переживают ребут** — tmpfs очищается. Если юзер ребутнет роутер до срабатывания cron-rollback'а, скрипт исчезнет, crontab-entry останется висеть мёртвым. Для критичных rollback'ов класть скрипт в `/usr/local/bin/` или `/root/`. Либо принять что ребут = ручное восстановление из `config.json.bak-*`.
3. **Любой `xray restart` на этом роутере = пауза без интернета у клиентов** (минимум секунды, при проблемах — до минуты). Перед рестартом предупреждать Сида.
4. Если возвращаться к XHTTP-миграции — делать не «добавить outbound в balancer», а сразу полную замену по плану ниже (Reality fallbacks, gRPC снести), чтобы число outbound'ов не росло. И отдельно сначала проверить рабочий клиентский XHTTP-конфиг (через `bo`/тестовое правило).

### Инцидент: silent-failure watchdog'а, 29 часов без алертов (2026-05-13)

**Симптом**: в ~16:20 у Сида отвалились все клиенты, бот отчитался («ALL_DEAD → proxy-grpc») **только после ручного ребута роутера в 16:23**. Предыдущий алерт был сутками раньше — 2026-05-12 в 10:59 («proxy-grpc → ALL_DEAD»), сразу после рестарта xray для GOMEMLIMIT. То есть бот молчал 29 часов, пока всё на самом деле сначала работало (Сид сидел в Discord), а потом сломалось.

**Корень**: 
1. `xray-tg-watchdog.sh` опирается на `xray api bi vpn-balancer` (опрос внутреннего api-inbound на 127.0.0.1:10085) и алертит только при **смене состояния**. При первом стартовом transient'е watchdog зафиксировал `ALL_DEAD`, записал в state-файл. Дальше `xray api bi` по неизвестной причине продолжал отдавать пусто (вероятно api-inbound стабильно деградировал, при том что data plane через TProxy жил) → state каждый раз ALL_DEAD == previous ALL_DEAD → молчок.
2. `xray-selfheal.sh` v1 имел правильный end-to-end curl-probe через прокси 8118, но **гейтил действие** на `active != safe`. У Сида active всегда == safe (мы старательно туда копируем после удачных деплоев) → selfheal каждый раз молча выходил.
3. `xray-memguard.sh` триггерится только по `MemAvailable < 8MB`. Сценарий «xray висит, но RAM в норме» им не покрыт.

**Что починили (`xray-selfheal.sh` v2)**: переписан в полноценный end-to-end watchdog. Логика:
- Каждые 2 мин (cron */2) curl через 8118 → gstatic/204, 3 попытки за ~25с.
- Probe провалился → **рестарт xray безусловно** (без условия config diff — освобождает heap, сбрасывает api/runtime).
- Через 20с probe ещё раз. Ок → TG «🛠️ auto-recovered».
- Не ок И конфиг != .safe → откат на .safe + рестарт → probe → TG «⚠️ откатил конфиг».
- Не ок И конфиг уже .safe → TG «🚨 не могу починить, вмешайся».
- Cooldown 15 мин между действиями (не зацикливаться, если сервер реально лёг).

В сегодняшнем сценарии Сид получил бы TG-нотификацию через 3-4 минуты после фриза и (вероятно) восстановление без ручного ребута.
