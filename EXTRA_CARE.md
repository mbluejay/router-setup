...
**Что пошло не так**: после `/etc/init.d/xray restart` для применения нового конфига TProxy лёг на ~5-8 секунд (с тремя outbound'ами observatory инициализируется дольше + RAM-churn на тесном роутере). Сид не дождался восстановления, ребутнул роутер. После ребута всё поднялось нормально (xray, прокси, интернет) — XHTTP сам по себе **не сломал** ничего, error.log после старта чистый. Но Сид решил не рисковать → `proxy-xhttp` outbound удалён с роутера, конфиг откатан на pre-xhttp. Серверный inbound id=6 на 2087 оставлен спать (ничего не ест, удаляется через панель при желании).

**Уроки**:
1. **Rollback-скрипты в `/tmp` не переживают ребут** — tmpfs очищается. Если юзер ребутнет роутер до срабатывания cron-rollback'а, скрипт исчезнет, crontab-entry останется висеть мёртвым. Для критичных rollback'ов класть скрипт в `/usr/local/bin/` или `/root/`, не в `/tmp`. Либо принять что ребут = ручное восстановление из `config.json.bak-*`.
2. **Любой `xray restart` на этом роутере = ~5-8с без интернета у клиентов.** Перед рестартом предупреждать. С 3+ outbound'ами или под нагрузкой может быть дольше. Возможно стоит вынести observatory probe в отдельный процесс или увеличить `probeInterval`, чтобы старт был легче.
3. Если возвращаться к XHTTP-миграции — делать не «добавить 4-й outbound в balancer», а сразу полную замену по плану ниже (Reality fallbacks, gRPC снести), чтобы число outbound'ов не росло.
